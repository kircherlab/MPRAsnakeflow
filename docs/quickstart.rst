...
4. Run MPRAsnakeflow
  ...
  Be sure that the files, :code:`experiment.csv` and the :code:`config.yaml` are correct. All fastq files for the count/experiment part must be in the same folder given by the :code:`data_folder` option. Please specify your barcode length and umi-length with :code:`bc_length` and :code:`umi_length`.
