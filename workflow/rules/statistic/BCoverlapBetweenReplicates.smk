...
rule statistic_overlapBCs:
    ...
    input:
        lambda wc: expand(
            "results/{{project}}/{{raw_or_assigned}}/{{condition}}_{replicate}_{{type}}_final_counts_{{sampling}}.tsv.gz",
            replicate=getReplicatesOfCondition(wc.project, wc.condition),
        ),
    output:
        "results/{project}/stats/{raw_or_assigned}/overlapBCandCounts_{condition}_{type}_{sampling}.tsv",
    params:
...
rule statistic_combine_overlapBCs_stats_raw:
    ...
    output:
        report(
            "results/{project}/stats/statistic_overlapBCs_counts_{sampling}.tsv",
...
rule statistic_combine_overlapBCs_stats_assigned:
    input:
        stats=lambda wc: expand(
            "results/{{project}}/stats/assigned_counts/{{assignment}}/overlapBCandCounts_{condition}_{type}_{{sampling}}.tsv",
            type=["DNA", "RNA"],
            condition=getConditions(wc.project),
        ),
    output:
        report(
            "results/{project}/stats/statistic_overlapBCs_assigned_counts_{assignment}_{sampling}.tsv",
