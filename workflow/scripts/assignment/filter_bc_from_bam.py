...
@click.option('--min_mapping_quality',
                '-q',
                ('min_mapping_quality'),
                required = False,
                default = 1,
...
def main(identity_threshold, mismatches_threshold, use_expected_alignment_length, expected_alignment_length, min_mapping_quality, bamfile, verbose, output_path):
  # helpful functions
  ...
  def get_number_of_matches_per_strand(read):
    ...
    # split ";" on right side of XA tag (XA: alternative alignment;
  ...
  def get_barcode(read):
    ...
    try:
      # XI: Barcode sequence (from the fastq headers resulting from the -C option of BWA)
