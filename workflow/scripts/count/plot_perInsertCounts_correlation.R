...
if ("label" %in% names(opt)) {
    label_f <- as.data.frame(
        read.table(opt$label, sep = "\t", header = T, stringsAsFactors = F)
    )
...
plot_correlations_dna <- function(data, condition, r1, r2, name) {
    dna_p <- ggplot(data, aes(dna_normalized_log2.x, dna_normalized_log2.y)) +
        ...
        xlab(
            sprintf(
                paste(
                    "log2 Normalized DNA count per insert,\n replicate",
                    r1
                )
            )
        ) +
        ...
        geom_text(
            ...
            label = sprintf(
                "   r = %.2f",
                cor(data$dna_normalized_log2.x, data$dna_normalized_log2.y,
                    method = "pearson"
                )
            ), size = 10
        ) +
...
plot_correlations_rna <- function(data, condition, r1, r2, name) {
    rna_p <- ggplot(data, aes(rna_normalized_log2.x, rna_normalized_log2.y)) +
        ...
        geom_text(
            ...
            label = sprintf(
                "   r = %.2f",
                cor(data$rna_normalized_log2.x, data$rna_normalized_log2.y,
                    method = "pearson"
                )
            ), size = 10
        ) +
...
readData <- function(file) {
    data <- read.table(file,
        ...
        mutate(
            ...
            rna_normalized_log2 = log2(rna_normalized), ratio_log2 = log2(ratio)
        )
    return(data)
}
...
for (n in 1:(data %>% nrow())) {
    ...
    assigned_counts <- all %>% filter(
        replicate == rep
    )

    # Histograms
    intercept <- median(assigned_counts$n_obs_bc)
    hist_plot_list[[n]] <- ggplot(assigned_counts, aes(x = n_obs_bc)) +
...
ggsave(sprintf("%s_all_barcodesPerInsert_box.png", outdir), box_plot)
...
ggsave(sprintf("%s_group_barcodesPerInsert_box.png", outdir), box_plot_insert)

box_plot_insert_thresh <- do.call("plot_grid", c(box_plot_insert_thresh_list))
ggsave(
